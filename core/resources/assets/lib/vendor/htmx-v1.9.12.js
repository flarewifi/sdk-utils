...
(function (root, factory) {
  ...
  return function () {
    ...
    var htmx = {
      ...
      _: internalEval,
      createEventSource: function createEventSource(url) {
    ...
    var scanForwardQuery = function scanForwardQuery(start, match) {
    ...
    var scanBackwardsQuery = function scanBackwardsQuery(start, match) {
      var results = getDocument().querySelectorAll(match);
    ...
    function stringHash(string, hash) {
      ...
      while (_char < string.length) {
        hash = (hash << 5) - hash + string.charCodeAt(_char++) | 0; // bitwise or ensures we have a 32-bit int
      }
      return hash;
    }
    ...
    function boostElement(elt, nodeData, triggerSpecs) {
      if (elt.tagName === "A" && isLocalLink(elt) && (elt.target === "" || elt.target === "_self") || elt.tagName === "FORM") {
        ...
        var _a2 = triggerSpecs;
        var _f2 = function _f2(triggerSpec) {
    ...
    function addEventListener(elt, handler, nodeData, triggerSpec, explicitCancel) {
      ...
      if (triggerSpec.changed) {
        var _a3 = eltsToListenOn;
        var _f3 = function _f3(eltToListenOn) {
    ...
    function processSSESwap(elt, sseEventName) {
      ...
      if (sseSourceElt) {
        ...
        var _sseListener = function sseListener(event) {
          if (maybeCloseSSESource(sseSourceElt)) {
            return;
          }
          if (!bodyContains(elt)) {
            sseEventSource.removeEventListener(sseEventName, _sseListener);
            return;
          }
        ...
        getInternalData(elt).sseListener = _sseListener;
        sseEventSource.addEventListener(sseEventName, _sseListener);
      } else {
    ...
    function processSSETrigger(elt, handler, sseEventName) {
      ...
      if (sseSourceElt) {
        var sseEventSource = getInternalData(sseSourceElt).sseEventSource;
        var _sseListener2 = function sseListener() {
          if (!maybeCloseSSESource(sseSourceElt)) {
            if (bodyContains(elt)) {
              handler(elt);
            } else {
              sseEventSource.removeEventListener(sseEventName, _sseListener2);
            }
          }
        };
        getInternalData(elt).sseListener = _sseListener2;
        sseEventSource.addEventListener(sseEventName, _sseListener2);
      } else {
        triggerErrorEvent(elt, "htmx:noSSESourceError");
      }
    }
    ...
    function processVerbs(elt, nodeData, triggerSpecs) {
      ...
      forEach(VERBS, function (verb) {
        if (hasAttribute(elt, 'hx-' + verb)) {
          ...
          nodeData.path = path;
          nodeData.verb = verb;
          var _a4 = triggerSpecs;
          var _f4 = function _f4(triggerSpec) {
    ...
    function evalScript(script) {
      if (!script.htmxExecuted && htmx.config.allowScriptTags && (script.type === "text/javascript" || script.type === "module" || script.type === "")) {
    ...
    function findHxOnWildcardElements(elt) {
      ...
      if (document.evaluate) {
        var iter = document.evaluate('.//*[@*[ starts-with(name(), "hx-on:") or starts-with(name(), "data-hx-on:") or' + ' starts-with(name(), "hx-on-") or starts-with(name(), "data-hx-on-") ]]', elt);
    ...
    function findElementsToProcess(elt) {
      if (elt.querySelectorAll) {
        ...
        var results = elt.querySelectorAll(VERB_SELECTOR + boostedSelector + ", form, [type='submit'], [hx-sse], [data-hx-sse], [hx-ws]," + " [data-hx-ws], [hx-ext], [data-hx-ext], [hx-trigger], [data-hx-trigger], [hx-on], [data-hx-on]");
        return results;
      } else {
        return [];
      }
      ...
      if (nodeData.initHash !== attributeHash(elt)) {
        ...
        if (!hasExplicitHttpAction) {
          if (getClosestAttributeValue(elt, "hx-boost") === "true") {
            ...
            var _a5 = triggerSpecs;
            var _f5 = function _f5(triggerSpec) {
    ...
    function processInputValue(processed, values, errors, elt, validate) {
      ...
      if (shouldInclude(elt)) {
        ...
        if (elt.multiple && elt.tagName === "SELECT") {
          var _a6 = toArray(elt.querySelectorAll("option:checked"));
    ...
    function issueAjaxRequest(verb, path, elt, event, etc, confirmed) {
      ...
      if (confirmed === undefined) {
        var issueRequest = function issueRequest(skipConfirmation) {
      ...
      if (beforeSwapDetails.shouldSwap) {
        ...
        var doSwap = function doSwap() {
          try {
            ...
            var doSettle = function doSettle() {
    ...
    function extensionBase() {
      return {
        ...
        isInlineSwap: function isInlineSwap(swapStyle) {
          return false;
        },
