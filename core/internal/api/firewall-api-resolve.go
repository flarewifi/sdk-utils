...
// dnsServers is the list of DNS servers queried in parallel for hostname resolution.
...
func (self *FirewallApi) ResolveHostnameToIps(hostname string) ([]string, error) {
	...
	for _, dnsServer := range dnsServers {
		...
		go func(server string) {
			...
			for _, ip := range ipList {
				ips = append(ips, normalizeIP(ip.String()))
			}
	...
	return nil, fmt.Errorf("failed to resolve hostname %s: all %d DNS servers failed (last error: %v), no cached data available", hostname, len(dnsServers), lastErr)
}

// normalizeIP normalizes an IP string, converting IPv4-mapped IPv6 addresses
...
// sortIPs sorts IP addresses in-place: IPv4 addresses first, then IPv6,
