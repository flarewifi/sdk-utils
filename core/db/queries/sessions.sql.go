...
const sessionSummary = `-- name: SessionSummary :one
SELECT
...
type SessionSummaryRow struct {
	DeviceID          pgtype.UUID
	TotalSessions     int64
	RemainingTimeSecs int32
	RemainingDataMb   float64
}

func (q *Queries) SessionSummary(ctx context.Context, deviceID pgtype.UUID) (SessionSummaryRow, error) {
	row := q.db.QueryRow(ctx, sessionSummary, deviceID)
	var i SessionSummaryRow
	err := row.Scan(
		&i.DeviceID,
		&i.TotalSessions,
		&i.RemainingTimeSecs,
